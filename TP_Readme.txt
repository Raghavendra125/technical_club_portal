...
(b)Install MySQL:
  Download and install MySQL Server from [MySQL official website](https://dev.mysql.com/).
(c)Install Required npm Packages:
  by typing the command "npm install" in shell or terminal
...
(a). Create a MySQL database:
   by using command "CREATE DATABASE technical_club;"
(b). Import the database schema (if provided as an SQL file):
   "mysql -u <username> -p technical_club < database/schema.sql"
...
(d).create the tables in the database:
  after creating and connecting to database use "technical_club_db_creation.sql" file provided in the database folder to create and insert data into tables.
